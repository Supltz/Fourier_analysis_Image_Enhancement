...
parser.add_argument('--description', default="Unet_Facade_focal", type=str)
...
parser.add_argument('--data_path', default="./CMP_facade_DB_base/base", type=str)
parser.add_argument('--label_path', default="./CMP_facade_DB_base/base",type=str)
...
def val(epoch):
    ...
    for idx, data in enumerate(tqdm(val_dataloader)):
        ...
        # store the result
    if epoch % 4 == 0:
        save_image(uw_img.cpu().data, './results_unet_facade_focal/uw_img_{}.png'.format(epoch))
        save_image(cl_img.cpu().data, './results_unet_facade_focal/gt_img_{}.png'.format(epoch))
        save_image(u_out.cpu().data, './results_unet_facade_focal/outputs_img{}.png'.format(epoch))
        save_image(tar_abs, './results_unet_facade_focal/gt_fft{}.png'.format(epoch))
        save_image(out_abs, './results_unet_facade_focal/out_fft{}.png'.format(epoch))
        save_image(uw_abs, './results_unet_facade_focal/uw_fft{}.png'.format(epoch))
...
if __name__ == "__main__":
    ...
    #make paths
    if not os.path.exists('./results_unet_facade_focal'):
        os.mkdir('./results_unet_facade_focal')
    ...
    train_dataset = Facade(data_path,
                                 label_path,
                                 mode='train')

    val_dataset = Facade(data_path,
                               label_path,
                               mode='val')
