...
parser.add_argument('--description', default="Unet", type=str)
...
def val(epoch):
    ...
    # store the result
    if epoch % 4 == 0:
        save_image(uw_img.cpu().data, './results_unet/uw_img_{}.png'.format(epoch))
        save_image(cl_img.cpu().data, './results_unet/gt_img_{}.png'.format(epoch))
        save_image(u_out.cpu().data, './results_unet/outputs_img{}.png'.format(epoch))
        save_image(tar_abs, './results_unet/gt_fft{}.png'.format(epoch))
        save_image(out_abs, './results_unet/out_fft{}.png'.format(epoch))
        save_image(uw_abs, './results_unet/uw_fft{}.png'.format(epoch))
...
if __name__ == "__main__":
    ...
    #make paths
    if not os.path.exists('./results_unet'):
        os.mkdir('./results_unet')
