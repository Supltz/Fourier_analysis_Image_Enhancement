...
parser.add_argument('--description', default="Combine_Facade", type=str)
...
def val(epoch):
    ...
    for idx, data in enumerate(tqdm(val_dataloader)):
        ...
        # store the result
    if epoch % 4 == 0:
        save_image(uw_img.cpu().data, './results_combine_facade/uw_img_{}.png'.format(epoch))
        save_image(cl_img.cpu().data, './results_combine_facade/gt_img_{}.png'.format(epoch))
        save_image(u_out, './results_combine_facade/outputs_img{}.png'.format(epoch))
        save_image(tar_abs, './results_combine_facade/gt_fft{}.png'.format(epoch))
        save_image(output_abs, './results_combine_facade/out_fft{}.png'.format(epoch))
        save_image(uw_abs, './results_combine_facade/uw_fft{}.png'.format(epoch))
...
if __name__ == "__main__":
    ...
    #make paths
    if not os.path.exists('./results_combine_facade'):
        os.mkdir('./results_combine_facade')
